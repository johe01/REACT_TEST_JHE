...
const BoardUpdateForm = ({ 
  ...
  const [fileIdList, setFileList] = useState([]) // 선택 삭제 id 목록
  ...
  const handleCheckedFileDelete = (id) => {
    const check = window.confirm(`선택한 ${fileIdList.length}개의 파일을 정말로 삭제하시겠습니까?`)
    if( check ) {
      deleteCheckedFiles(fileIdList)
      setFileList([])       // 삭제할 id 리스트 초기화
    }
  }

  // ✅ 체크박스 클릭 핸들러
  const checkFileId = (id) => {
    console.log(`그냥 체크한 아이디 : ${id}`)
    ...
    <div className="container">
      ...
      <table>
        <tbody>
          ...
          <tr>
            <td colSpan={2}>
              <textarea cols={40} rows={10} defaultValue={content}
            ...
            <td colSpan={2}>
              {
                fileList.map((file) => (
                  <div className='flex-box' key={file.id}>
                    ...
                    <div className='item'>
                      <button className='btn' onClick={ () => onDownload(file.id, file.originName) } ><FileDownloadIcon /></button>
